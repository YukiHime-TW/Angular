...
== HTML模板 (Template)
Angular 元件的模板文件，負責定義類別元件的視圖結構，透過 Angular 的模板語法實現資料的顯示、條件渲染、事件處理等功能，並動態更新畫面，與 app.component.ts 當中的邏輯緊密結合，實現互動式的使用者界面。

== HTML模板常用的語法

=== Interpolation
用來顯示元件類中的資料，在HTML模板使用{{}}即可取得屬性值
...
Angular 處理事件時，會將事件與元件實例綁定，與傳統 JavaScript 的事件處理不同。
this會指向該元件
