...
**Web 元件的關鍵屬性** 是「**封裝**」，它允許開發者將標記結構(markup)、樣式(style)和行為(behavior)封裝起來，並與頁面上的其他代碼獨立。這意味著 Web 元件具備以下優點：
...
- Shadow DOM 讓Web 元件開發變得更加靈活和高效，避免樣式衝突和行為干擾。
...
| **目的** | 隱藏元件內部的結構和樣式，防止外部樣式影響，並且避免樣式衝突。 | 擴展 HTML 語言，創建自定義的元素和行為。
